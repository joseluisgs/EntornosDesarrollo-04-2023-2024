...
fun main(args: Array<String>) {
    ...
    if (args.size != 1) {
        logger.error { "No se ha introducido ningún dni" }
...
fun comprobarDniSinExpresionesRegulares(dni: String): Boolean {
    logger.debug { "Comprobando dni sin expresiones regulares" }
    ...
    logger.debug { "Letra correcta: $letraCorrecta" }
...
fun comprobarDniConExpresionesRegulares(dni: String): Boolean {
    if (!Regex("[0-9]{8}[A-Z]").matches(dni)) {
        logger.debug { "El dni no cumple el patrón" }
        return false
    }
    val letra = dni.last()
